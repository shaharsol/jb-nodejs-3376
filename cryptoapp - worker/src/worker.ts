...
async function work() {
    try {
        ...
        const response = await axios(`http://${config.get('worker.api.host')}:${config.get('worker.api.port')}/api/symbols`);
        console.log('got symbols to scrape from app microservice');
        // const uniqueSymbols = await getUserSymbolModel().getUniqueSymbols();
        const uniqueSymbols = response.data;
